...
bool readOption(struct Portfolio* port, std::string& line, double &maturity, double &r) {
    struct Option opt;
    opt.r = r;
    opt.value = 0;
    ...
    port->option_list.push_back(std::move(opt));
    return true;
}
...
void Reader::readPortfolio(struct Portfolio* port, double maturity, double r) const{
    ...
    while (getline(fp, line)) {
        if (!readOption(port, line, maturity, r)) {
            perror("error: ");
            return;
        }
    }
